...
(***NOTE*** for people who already know modal logic: the interpretation of `□` is not merely superficially different from Saul Kripke's `Kx`: it makes a difference to what we can plausibly go on to use as an axiom. Because in epistemic logic we have the truth axiom: `Kxp → p`, whereas we will reject the analogous axiom. This is why I'm using the term "evidential logic" for what I'm doing: Kripke has already used "epistemic logic".)
